...
/*
    ...
    - 순서 존재
    - 배열 디스트럭처링에 배열의 길이를 초과한 변수를 선언하면 초과한 범위에 해당하는 변수는 undefined가 지정
*/
...
/*
    spread 연산자를 활용
    - 배열 디스트럭처링 마지막에 스프레드 연산자를 사용하면 남은 모든 요소로 새로운 배열 생성
*/
const [anotherFruit, ...otherFruits] = fruits;
console.log(anotherFruit, otherFruits);

//const[...otherFruits, anotherFruit] = fruits;
...
/*
    spread 연산자를 활용
    - 객체 디스트럭처링 마지막에 스프레드 연산자를 사용하면 나머지 모든 속성을 묶어서 새로운 객체 생성
*/
const {birth, ...leftKing} = king;
console.log(birth);
console.log(leftKing);
