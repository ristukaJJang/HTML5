...
// 선대입 후증가 
result = single++ * 8; //result = 0
// 선증가 후대입
result = ++single * 8; //result = 16
// 선대입 후감소
result = single-- * 8; //result = 16
// 선감소 후대입
result = --single * 8; //result = 0
...
/*
    - (+) 단항 연산자는 숫자 타입이 아닌 피연산자에 사용하여 숫자 타입으로 변환할 수 있음
*/
...
/*
    ...
    - 숫자 타입이 아닌 피연산자를 사용하면 부호가 반전된 숫자 타입으로 변환
*/
console.log(-10);
console.log(-'10');
console.log(-false);
...
/*
    문자열 연결 연산자
    - (+) 이항 연산자의 피연산자 중 하나라도 문자열이면 연결 연산자로 사용
*/
console.log(1+'1');
...
/*
    ...
    - !== : 불일치 비교
*/
// 동등, 부등, 비교는 자동형변환 수행 후 연산 진행 (데이터 타입 영향 X)
console.log(10 == '10');
// 일치, 불일치 비교는 형변환을 수행하지 않음 (데이터 타입 영향 O)
console.log(10 === '10');
...
var result = '';
var number = 9;
if (number > 0) result = '양수';
else {
    if(number < 0) result = '음수';
    else result = '영';
}
result = number > 0 ? '양수' 
    : (number < 0) ? '음수' 
